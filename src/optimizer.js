...
export function optimize(query) {
  ...
  if (['concat', 'intersect', 'union', 'except'].some(clause => keys.includes(clause))) {
    return new SqlQuery(source, query._schema, query._clauses);
  }
  ...
  if (source_highest_key === 0) {
    ...
    return new SqlQuery(source._source, query._schema, {...query._clauses, ...(where.length === 0 ? {} : {where})});
  }
  ...
  if (query_lowest_key > source_highest_key) {
    return new SqlQuery(source._source, query._schema, {...source._clauses, ...query._clauses});
  }
