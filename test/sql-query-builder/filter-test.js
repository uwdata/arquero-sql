...
tape('Sql-query-builder: filter', t => {
  const filterForWhere = base.filter([d => d.a > 0]);

  const filterForHaving = baseWithGroupBy.filter([d => op.mean(d.a) > 0, d => d.b > 0]);
  ...
  t.throws(() => base.filter([d => op.mean(d.a) > 0]), 'Cannot fillter using aggregate operations without groupby');
